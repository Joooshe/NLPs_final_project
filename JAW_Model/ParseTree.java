...
/**
 * A class representing a syntactic parse tree. The parse tree is a recursive
 * structure.
 * A parse tree consists of a label and, if it is not a terminal, children that
 * are also
 * parse trees.
 * 
 * @author Dave Kauchak
...
public class ParseTree {
	...
	/**
	 * Construct a new parse tree from a single line, parenthesized version of the
	 * parse tree.
	 * 
	 * @param treeString A single line containing a parenthesized version of a parse
	 *                   tree
	 */
	public ParseTree(String treeString) {
		...
		if (treeString.contains("(")) {
			...
			// when we get down the the part of speech tag, create the POS node as a
			// non-terminal
	...
	/**
	 * Construct a new parse tree without any children (though it still may be a
	 * non-terminal
	 * with children yet unattached)
	 * 
	 * @param label    the constituent label for this node (or the word if it's a
	 *                 terminal)
